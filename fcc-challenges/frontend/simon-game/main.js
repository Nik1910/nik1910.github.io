...
// Starts the new game when the "go" button is clicked, and change text of the button
$("#go").click(function(){
  newGame(); 
  $(this).html("Reset");
});
...
// Show the player moves to repeat
function showMoves(){
  
// Stores value to loop through "currentGame"  
  var i = 0;
  
// Sets interval to display every 0,6 sec moves 
...
// Otherwise, if the player does everything right and "count" isn't 20, it tells him that to go on and adds 1 to "count"      
      else{
        right();
        nextLevel();
      }
...
// Turns on strict mode  
  if(game.strict === false){
    game.strict = true;
    $("#switcher").html("ON");
    $("#strict").removeClass("btn-success").addClass("btn-warning");
  }
...
// Turns off strict mode  
    ...
    game.strict = false;
    $("#switcher").html("OFF");
    $("#strict").removeClass("btn-warning").addClass("btn-success");
  }
...
function right(){
  $("#status").removeClass("btn-danger").addClass("btn-info");
