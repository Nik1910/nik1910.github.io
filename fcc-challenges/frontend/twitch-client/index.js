...
$(document).ready(function(){
  ...
  var users1 =  ["ESL_SC2", "OgamingSC2", "cretetion", "freecodecamp", "storbeck", "habathcx", "RobotCaleb", "noobs2ninjas"];
    ...
    // Loop through all channels for data
  ...
  function ajax1(user1){
    $.ajax({
      url: "https://api.twitch.tv/kraken/channels/" + user1,
      type: "GET",
      async: false,
      headers: {
...
// Create new div for every channel online
...
// If a channel is closed,the error function'll let users know about it     
      error: function(){
        ...
        $("#channels").prepend("<div class='well'>"+"<img class='img-responsive' src='http://res.cloudinary.com/dwxepusiu/image/upload/v1523503494/closed-sign_utrm1x.png' style='width:200px;height:130px'/>"+"<p class='off'></p>"+"<p class='text-center'>"+this.url.substr(this.url.lastIndexOf("/") + 1).charAt(0).toUpperCase()+this.url.substr(this.url.lastIndexOf("/") + 1).slice(1) +"</p></div>");
      }
...
// One more ajax request to find out which channels are currently offline 
  function ajax2(user2){
    $.ajax({
     url: "https://api.twitch.tv/kraken/streams/" + user2,
    type: "GET",
    async: false,
    headers: {
        "Client-ID": "c5m7bh23wta20firvrtyrmamue346y"
...
// When the page is loaded,focus on the "all" button 
  
  window.onload = function focusOnAll() {
    $("#all").focus();
  };
